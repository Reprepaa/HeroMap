...
document.addEventListener('DOMContentLoaded', () => {
   ...
   if (headline && paragraph) {
      ...
      const textSets = [{
            h1: "WebGIS Informatif <br>Taman Makam Pahlawan",
            p: "Platform ini menyajikan informasi spasial mengenai lokasi makam pahlawan, profil tokoh, serta kondisi kawasan melalui peta digital interaktif."
         },
         {
            h1: "Pemetaan Digital <br> Kawasan Taman Makam Pahlawan",
            p: "Data geospasial kawasan Taman Makam Pahlawan ditampilkan dalam bentuk peta interaktif untuk memudahkan eksplorasi lokasi makam dan informasi terkait."
         },
         {
            h1: "Jelajahi Informasi<br>Para Pahlawan",
            p: "Melalui WebGIS ini, pengunjung dapat melihat lokasi makam, membaca profil tokoh pahlawan, serta memahami sejarah kawasan secara lebih mudah."
         }
      ];
...
document.getElementById("filterBlok").addEventListener("change", function() {
    let filter = this.value.toLowerCase();
    let rows = document.querySelectorAll("#dataTable tbody tr");

    rows.forEach(row => {
        let blok = row.innerText.toLowerCase();
        row.style.display = blok.includes(filter) || filter === "" ? "" : "none";
    });
});
// MAP - dikunci, tidak bisa digeser
var map = L.map('map', {
    ...
    touchZoom: false,       // nonaktifkan pinch zoom
    zoomControl: false,     // sembunyikan tombol +/-
    keyboard: false         // nonaktifkan keyboard navigation
}).setView([-7.289534237418184, 112.70834801662886], 20);
// Setelah load GeoJSON, fit ke bounds makam
map.fitBounds(makamLayer.getBounds(), {
    padding: [20, 20]
});

// Kunci di posisi itu, tidak bisa kemana-mana
